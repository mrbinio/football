      ...
      {showModal && (
        <div style={{
          ...
          <form onClick={e => e.stopPropagation()} onSubmit={handleSubmit} style={{
            ...
            <label style={{ fontSize: 13, color: 'var(--text2)', display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', padding: '8px 0' }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
              ...
              <button type="button" onClick={() => setShowModal(false)} style={{ background: 'rgba(255,255,255,0.05)', color: 'var(--text2)', border: '1px solid rgba(255,255,255,0.1)', flex: 1 }}>
                Cancel
              </button>
            </div>
          </form>
        ...
        {players.sort((a, b) => a.number - b.number).map(p => (
          <div key={p.id} onClick={() => openEdit(p)} style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 11, color: 'var(--text3)' }}>#{p.number} · {p.shirtName} · {p.position}</div>
            </div>
